...
from nnRelationalVerify.common.network import LayerType
from nnRelationalVerify.util.util import compute_input_shapes
from nnRelationalVerify.relu.relu_transformer import ReLUTransformer
from nnRelationalVerify.common.dataset import Dataset
...
class IndividualAndRelationalBounds(ReLUTransformer):
    ...
    def check_lb_ub_correctness(self, lb, ub, layer_idx=None, logging=True):
    ...
    def back_substitution_IAR(self, layer_idx, pre_IARb=None, logging=True):
        ...
        for r_layer_idx in reversed(range(layer_idx + 1)):
            ...
            if back_prop_struct is not None:
                if r_layer_idx == 0:
                    ...
                    inp1_lbub_alarm = self.check_lb_ub_correctness(lb=new_inp1_lb, ub=new_inp1_ub, layer_idx=layer_idx, logging=logging)
                    inp2_lbub_alarm = self.check_lb_ub_correctness(lb=new_inp2_lb, ub=new_inp2_ub, layer_idx=layer_idx, logging=logging)
                    d_lbub_alarm = self.check_lb_ub_correctness(lb=new_d_lb, ub=new_d_ub, layer_idx=layer_idx, logging=logging)
        ...
        if self.refine_bounds_prop:
            ...
            inp1_lbub_alarm = self.check_lb_ub_correctness(lb=inp1_lb, ub=inp1_ub, layer_idx=layer_idx, logging=logging)
            inp2_lbub_alarm = self.check_lb_ub_correctness(lb=inp2_lb, ub=inp2_ub, layer_idx=layer_idx, logging=logging)
            d_lbub_alarm = self.check_lb_ub_correctness(lb=d_lb, ub=d_ub, layer_idx=layer_idx, logging=logging)
            if inp1_lbub_alarm or inp2_lbub_alarm or d_lbub_alarm:
                return None, None, None, None, None, None
    ...
    def update_bounds_IAR(self, rs_history=None, is_history=None, split_layer_list=None, logging=True):
        """
        1. replace the preactivation bounds from rs_history
        ...
        for layer_idx in range(start_layer_idx, len(self.net)):
            if self.net[layer_idx].type == LayerType.ReLU and layer_idx in split_layer_list:  # update bounds of preactivation layer
                if rs_history is not None:
                    temp_ds_info_list = [rs_info for rs_info in rs_history if rs_info.layer_idx == layer_idx]
                    for rs_info in temp_ds_info_list:
                        inp1_lb = self.inp1_lbs[layer_idx][rs_info.pos]
                        inp1_ub = self.inp1_ubs[layer_idx][rs_info.pos]
                        inp2_lb = self.inp2_lbs[layer_idx][rs_info.pos]
                        inp2_ub = self.inp2_ubs[layer_idx][rs_info.pos]
                        d_lb = self.d_lbs[layer_idx][rs_info.pos]
                        d_ub = self.d_ubs[layer_idx][rs_info.pos]

                        if '1' in rs_info.rs_type:
                            # update bounds
                            new_d_lb = d_lb
                            new_d_ub = rs_info.split_value
                        ...
                        elif '2' in rs_info.rs_type:
                            # update bounds
                            new_d_lb = rs_info.split_value
                            new_d_ub = d_ub
                            ...
                            raise ValueError("Unknown rs_type")
                    self.inp1_lbs[layer_idx][rs_info.pos] = new_inp1_lb
                    self.inp1_ubs[layer_idx][rs_info.pos] = new_inp1_ub
                    self.inp2_lbs[layer_idx][rs_info.pos] = new_inp2_lb
                    self.inp2_ubs[layer_idx][rs_info.pos] = new_inp2_ub
                    self.d_lbs[layer_idx][rs_info.pos] = new_d_lb
                    self.d_ubs[layer_idx][rs_info.pos] = new_d_ub
                elif is_history is not None:
                    temp_ns_info_list = [is_info for is_info in is_history if is_info.layer_idx == layer_idx]
                    for is_info in temp_ns_info_list:
                        inp1_lb = self.inp1_lbs[layer_idx][is_info.pos]
                        inp1_ub = self.inp1_ubs[layer_idx][is_info.pos]
                        inp2_lb = self.inp2_lbs[layer_idx][is_info.pos]
                        inp2_ub = self.inp2_ubs[layer_idx][is_info.pos]
                        d_lb = self.d_lbs[layer_idx][is_info.pos]
                        d_ub = self.d_ubs[layer_idx][is_info.pos]

                        if 'A1' in is_info.is_type:
                            # update bounds
                            new_inp1_lb = inp1_lb  # lb1 = lb2 + d_lb
                            new_inp2_lb = inp2_lb  # lb2 = lb1 - d_ub
                    ...
                    self.inp1_lbs[layer_idx][is_info.pos] = new_inp1_lb
                    self.inp1_ubs[layer_idx][is_info.pos] = new_inp1_ub
                    self.inp2_lbs[layer_idx][is_info.pos] = new_inp2_lb
                    self.inp2_ubs[layer_idx][is_info.pos] = new_inp2_ub
                    self.d_lbs[layer_idx][is_info.pos] = new_d_lb
                    self.d_ubs[layer_idx][is_info.pos] = new_d_ub

            curr_inp1_lb, curr_inp1_ub, curr_inp2_lb, curr_inp2_ub, curr_d_lb, curr_d_ub = self.back_substitution_IAR(layer_idx=layer_idx, logging=logging)
    ...
    def run_full_back_substitution_IAR(self, logging=True):
        """
    ...
    def run(self, logging=True):
        with torch.no_grad():
            ...
            return self.run_full_back_substitution_IAR(logging=logging)
