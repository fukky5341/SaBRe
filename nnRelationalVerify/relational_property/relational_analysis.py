from nnRelationalVerify.common import Status
import numpy as np
...
def relational_analysis_back(IARb, RelAna, log_file="log", RS_history=None, IS_history=None):
    ...
    if RS_history is not None and IS_history is not None:
        raise ValueError("Both RS_history and IS_history are provided. Only one must be provided.")
    if RelAna.relational_prop == RelationalProperty.GLOBAL_ROBUSTNESS:
        status, delta_out_dist = RelAna.run_global_robustness_analysis(
            IARb=IARb, RS_history=RS_history, IS_history=IS_history, log_file=log_file)
...
class RelationalAnalysis:
    ...
    def update_lp_model(self, model, RS_history=None, IS_history=None, inp1_lbs=None, inp1_ubs=None, inp2_lbs=None, inp2_ubs=None, d_lbs=None, d_ubs=None):

        # $ add split constraints
        if RS_history is not None and IS_history is not None:
            raise ValueError("Both RS_history and IS_history are provided. Only one must be provided.")

        elif RS_history is not None:
            for rs_info in RS_history:
                rs_type = rs_info.rs_type
                if "1" in rs_type:
                    self.add_ds_1_grb_constraints(model=model, RSInfo=rs_info, inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs,
                                                  inp2_lbs=inp2_lbs, inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
                elif "2" in rs_type:
                    self.add_ds_2_grb_constraints(model=model, RSInfo=rs_info, inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs,
                                                  inp2_lbs=inp2_lbs, inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
                else:
                    raise ValueError(f"Unsupported RS type: {rs_type}")
        elif IS_history is not None:
            for is_info in IS_history:
                is_type = is_info.is_type
                if "A1" in is_type:
                    self.add_ns_A1_grb_constraints(model=model, IS_info=is_info, inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs,
                                                   inp2_lbs=inp2_lbs, inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
                elif "A2" in is_type:
                    self.add_ns_A2_grb_constraints(model=model, IS_info=is_info, inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs,
                                                   inp2_lbs=inp2_lbs, inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
                elif "B1" in is_type:
                    self.add_ns_B1_grb_constraints(model=model, IS_info=is_info, inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs,
                                                   inp2_lbs=inp2_lbs, inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
                elif "B2" in is_type:
                    self.add_ns_B2_grb_constraints(model=model, IS_info=is_info, inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs,
                                                   inp2_lbs=inp2_lbs, inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
                else:
                    raise ValueError(f"Unsupported IS type: {is_type}")
        else:
            raise ValueError("Both RS_history and IS_history are None. At least one must be provided.")
    ...
    def add_ds_1_grb_constraints(self, model, RSInfo, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
        [d_lb, d_ub] -> [d_lb, split_value]
        """
        split_layer_idx = RSInfo.layer_idx
        split_pos = RSInfo.pos
        split_value = RSInfo.split_value
    ...
    def add_ds_2_grb_constraints(self, model, RSInfo, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
    ...
    def add_ns_A1_grb_constraints(self, model, IS_info, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
        [l1, u1] -> [l1, 0]
        """
        split_layer_idx = IS_info.layer_idx
        split_pos = IS_info.pos
        split_value = IS_info.split_value
        inp1_lb = inp1_lbs[split_layer_idx][split_pos].item()
        inp1_ub = inp1_ubs[split_layer_idx][split_pos].item()
        inp2_lb = inp2_lbs[split_layer_idx][split_pos].item()
        inp2_ub = inp2_ubs[split_layer_idx][split_pos].item()
    ...
    def add_ns_A2_grb_constraints(self, model, IS_info, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
        [l1, u1] -> [0, u1]
        """
        split_layer_idx = IS_info.layer_idx
    ...
    def add_ns_B1_grb_constraints(self, model, IS_info, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
        [l2, u2] -> [l2, 0]
        """
        split_layer_idx = IS_info.layer_idx
    ...
    def add_ns_B2_grb_constraints(self, model, IS_info, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
        [l2, u2] -> [0, u2]
        """
        split_layer_idx = IS_info.layer_idx
    ...
    def run_label_diff_lp_analysis(self, model, delta_out_grb_vars, IARb, log_file=""):
        ...
        if self.global_target:
            ...
            if delta_min is None:
                ...
                if self.threshold is not None and self.threshold < abs(diff):
    ...
    def run_global_robustness_analysis(self, IARb, RS_history=None, IS_history=None, log_file=""):
        if self.lp_analysis:
            ...
            if self.grb_model is None:
                ...
                temp_relAna = RelationalAnalysis()
                temp_relAna.duplicate(self)
                temp_relAna.grb_model = temp_relAna.update_lp_model(model=temp_relAna.grb_model, RS_history=RS_history, IS_history=IS_history,
                                                                    inp1_lbs=inp1_lbs, inp1_ubs=inp1_ubs, inp2_lbs=inp2_lbs,
                                                                    inp2_ubs=inp2_ubs, d_lbs=d_lbs, d_ubs=d_ubs)
