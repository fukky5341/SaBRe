...
class DualRelu():
    ...
    def get_lambda(self, inp1_v, inp2_v, delta_v, precise=False):
        ...
        if inp1_unstable.any() or inp2_unstable.any():
            ...
            if precise:
                l_inp1 = torch.where(case_5 & inp1_v_pos, temp_inp1, l_inp1)
                l_inp1 = torch.where(case_5 & inp1_one & inp1_v_neg, torch.ones_like(inp1_lb), l_inp1)
                l_inp1_d = torch.where(case_5 & d_v_pos, temp_inp1, l_inp1_d)
                l_inp1_d = torch.where(case_5 & d_v_neg, torch.ones_like(d_lb), l_inp1_d)
            else:
            ...
            if precise:
                l_inp2 = torch.where(case_6 & inp2_v_pos, temp_inp2, l_inp2)
                l_inp2 = torch.where(case_6 & inp2_v_neg, torch.ones_like(inp2_lb), l_inp2)
                l_inp2_d = torch.where(case_6 & d_v_neg, -temp_inp2, l_inp2_d)
                l_inp2_d = torch.where(case_6 & d_v_pos, -torch.ones_like(d_lb), l_inp2_d)
            else:
            ...
            if precise:
                l_inp1 = torch.where(case_7 & inp1_v_pos, temp_inp1, l_inp1)
                l_inp1 = torch.where(case_7 & inp1_one & inp1_v_neg, torch.ones_like(inp1_lb), l_inp1)
                l_inp1_d = torch.where(case_7 & d_v_pos, temp_inp1, l_inp1_d)
                l_inp1_d = torch.where(case_7 & d_v_neg, torch.ones_like(d_lb), l_inp1_d)
            else:
            ...
            if precise:
                l_inp2 = torch.where(case_8 & inp2_v_pos, temp_inp2, l_inp2)
                l_inp2 = torch.where(case_8 & inp2_v_neg, torch.ones_like(inp2_lb), l_inp2)
                l_inp2_d = torch.where(case_8 & d_v_neg, -temp_inp2, l_inp2_d)
                l_inp2_d = torch.where(case_8 & d_v_pos, -torch.ones_like(d_lb), l_inp2_d)
            else:
            ...
            if precise:
                l_inp1 = torch.where(case_9 & inp1_v_pos, temp_inp1, l_inp1)
                l_inp1 = torch.where(case_9 & inp1_one & inp1_v_neg, torch.ones_like(inp1_lb), l_inp1)
                l_inp2 = torch.where(case_9 & inp2_v_pos, temp_inp2, l_inp2)
                l_inp2 = torch.where(case_9 & inp2_v_neg, torch.ones_like(inp2_lb), l_inp2)
            else:
                l_inp1 = torch.where(case_9, temp_inp1, l_inp1)
                l_inp2 = torch.where(case_9, temp_inp2, l_inp2)
            case_9_pre_positive = case_9 & d_v_pos
            ...
            if precise:
                l_inp1 = torch.where(case_10 & inp1_v_pos, temp_inp1, l_inp1)
                l_inp1 = torch.where(case_10 & inp1_one & inp1_v_neg, torch.ones_like(inp1_lb), l_inp1)
                l_inp2 = torch.where(case_10 & inp2_v_pos, temp_inp2, l_inp2)
                l_inp2 = torch.where(case_10 & inp2_v_neg, torch.ones_like(inp2_lb), l_inp2)
            else:
                l_inp1 = torch.where(case_10, temp_inp1, l_inp1)
                l_inp2 = torch.where(case_10, temp_inp2, l_inp2)
            case_10_pre_negative = case_10 & d_v_neg
            ...
            if case_11.any():
                if precise:
                    l_inp1 = torch.where(case_11 & inp1_v_pos, temp_inp1, l_inp1)
                    l_inp1 = torch.where(case_11 & inp1_one & inp1_v_neg, torch.ones_like(inp1_lb), l_inp1)
                    l_inp2 = torch.where(case_11 & inp2_v_pos, temp_inp2, l_inp2)
                    l_inp2 = torch.where(case_11 & inp2_v_neg, torch.ones_like(inp2_lb), l_inp2)
                else:
                    l_inp1 = torch.where(case_11, temp_inp1, l_inp1)
                    l_inp2 = torch.where(case_11, temp_inp2, l_inp2)
                case_11_pre_negative = case_11 & d_v_neg
                case_11_pre_positive = case_11 & d_v_pos
