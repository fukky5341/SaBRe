...
class DualAnalysis:
    ...
    def estimate_relational_impact(self, dualnet_1, dualnet_2, RS_mode, layer_idx, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        """
        (requirement)
        ...
        for dual_network in [dualnet_1, dualnet_2]:
            split_scores, ds_conditions, ds_types = self.get_ds_merged_scores(RS_mode, dual_network.dual_net[idx-1].bias, idx, inp1_lbs[idx], inp1_ubs[idx],
        ...
        combined_scores = {}
        for rs_type in ds_types:
            combined_scores[rs_type] = split_score_list[0][rs_type] + split_score_list[1][rs_type]

        # arrange in descending order
        rs_order = self.arrange_in_descending_order(ds_conditions, combined_scores, idx, ds_types)

        return rs_order  # rs_order: [[rs_type, lubs_layer_idx, pos], ...]
    ...
    def estimate_relational_impact_ns(self, dualnet_1, dualnet_2, layer_idx, inp1_lbs, inp1_ubs, inp2_lbs, inp2_ubs, d_lbs, d_ubs):
        ...
        combined_scores = {}
        for is_type in ['A', 'B']:
            combined_scores[is_type] = split_scores_list[0][is_type] + split_scores_list[1][is_type]

        # arrange in descending order
        is_order = self.arrange_in_descending_order_ns(ns_conditions, combined_scores, idx)

        return is_order

    def get_ds_merged_scores(self, RS_mode, bias, preactivation_lubs_idx, inp1_lb, inp1_ub, inp2_lb, inp2_ub,
        ...
        if not PB.inp1_unstable.any() or not PB.inp2_unstable.any():
            return [], [], []
        if 'RS_dual_Z' == RS_mode:
            ds_conditions = self.sort_into_single(PB, 'RSZ')  # RSZ, common
            ds_types = ['RSZ']
        else:
            raise ValueError(f"Unsupported RS_mode: {RS_mode}")
    ...
    def sort_into_two(self, PB):  # PB = PreactivationBounds
        ...
        ds_conditions['RSM'] = ((1e-6 < PB.d_ub - PB.d_lb) & ((PB.inp1_lb < -1e-6) & (1e-6 < PB.inp1_ub))) | \
            ((1e-6 < PB.d_ub - PB.d_lb) & ((PB.inp2_lb < -1e-6) & (1e-6 < PB.inp2_ub)))
        ds_conditions['RSZ'] = ((PB.d_lb < 0) & (0 < PB.d_ub) & ((PB.inp1_lb < -1e-6) & (1e-6 < PB.inp1_ub))) | \
            ((PB.d_lb < 0) & (0 < PB.d_ub) & ((PB.inp2_lb < -1e-6) & (1e-6 < PB.inp2_ub)))

        ds_conditions['common'] = ds_conditions['RSM'] | ds_conditions['RSZ']

        return ds_conditions

    def sort_into_single(self, PB, rs_type):  # PB = PreactivationBounds
        ds_conditions = {}
        # conditions for sorting
        if rs_type == 'RSZ':
            ds_conditions['RSZ'] = ((PB.d_lb < 0) & (0 < PB.d_ub) & ((PB.inp1_lb < -1e-6) & (1e-6 < PB.inp1_ub))) | \
                ((PB.d_lb < 0) & (0 < PB.d_ub) & ((PB.inp2_lb < -1e-6) & (1e-6 < PB.inp2_ub)))
        ds_conditions['common'] = ds_conditions[rs_type]
    ...
    def get_split_scores_ds(self, old_score, bias, PB, ds_conditions, inp1_v, inp2_v, delta_v):
        ...
        rs: [dl, du] --> (1)[dl, new_d], (2)[new_d, du]
    ...
    def arrange_in_descending_order(self, ds_conditions, ds_scores, preactivation_lubs_idx, ds_types):
        ...
        ds_types = ['RSA', 'RSB', 'RSC', 'RSD'] or ['RSA', 'RSB', 'RSC', 'RSD', 'RSM', 'RSZ'] or ['RSM', 'RSZ']
        """
        sorted_scores = []
        for rs in ds_types:
            active_idx = torch.where(ds_conditions[rs])[0]
            if active_idx.numel() > 0:
                for idx in active_idx:
                    sorted_scores.append([rs, idx.item(),  ds_scores[rs][idx].item()])  # [rs_type, pos, score]
        # Sort by score in descending order
        sorted_scores.sort(key=lambda x: x[2], reverse=True)
        sorted_order = [[item[0], preactivation_lubs_idx, item[1]] for item in sorted_scores]  # candidate is [rs_type, lubs_layer_idx, pos]
    ...
    def arrange_in_descending_order_ns(self, ns_conditions, ds_scores, preactivation_lubs_idx):
        ...
        ns_names = ['A', 'B']
        sorted_scores = []
        for ind_sp in ns_names:
            active_idx = torch.where(ns_conditions[ind_sp])[0]
            if active_idx.numel() > 0:
                for idx in active_idx:
                    sorted_scores.append([ind_sp, idx.item(),  ds_scores[ind_sp][idx].item()])
        # Sort by score in descending order
        sorted_scores.sort(key=lambda x: x[2], reverse=True)
        sorted_order = []
        for item in sorted_scores:
            # item is [is_type, pos, score]
            candidate = [item[0], preactivation_lubs_idx, item[1]]
            # candidate is [is_type, lubs_layer_idx, pos]
